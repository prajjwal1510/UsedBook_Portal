...
function getBook()
{
    ...
    if (!isset($_GET['cat'])) {
        ...
        if (!isset($_GET['subject'])) {
            ...
            $get_book = "select * from book order by rand() DESC LIMIT 0,4";
...
<div class=\"col-md-3 \">
        ...
        ";
           ...
           echo" <div>
            <a href='all_request.php' class='btn btn-primary'>All Request</a>
</div>";
        }
    }
}
...
function getCatbook()
{
    ...
    if (isset($_GET['cat'])) {
        ...
        while ($row_cat = mysqli_fetch_array($run_cat_name)) {
            $cat_name = $row_cat['cat_title'];
        }

        $get_cat_book = "select * from book where book_category='$cat_name'";
...
function getSubjectBook()
{
    ...
    if (isset($_GET['subject'])) {
        ...
        global $con;
        $get_sub_name="select * from subject where sub_id='$subject_id'";
        $run_sub_name=mysqli_query($con,$get_sub_name);
        while ($row_sub = mysqli_fetch_array($run_sub_name)) {
            $sub_name = $row_sub['sub_name'];
        }

        $get_subject_book = "select * from book where book_subject='$sub_name'";
